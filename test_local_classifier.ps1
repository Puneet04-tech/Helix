...
try {
    ...
    Write-Host "✓ Response: analyzed=$($resp1.analyzed), reason=$($resp1.reason)" -ForegroundColor Green
} catch {
...
try {
    ...
    Write-Host "✓ Response: analyzed=$($resp2.analyzed), reason=$($resp2.reason)" -ForegroundColor Green
} catch {
    Write-Host "✗ Error: $_" -ForegroundColor Red
}
Start-Sleep -Milliseconds 500
...
try {
    ...
    Write-Host "✓ Response: analyzed=$($resp3.analyzed), reason=$($resp3.reason)" -ForegroundColor Green
} catch {
    Write-Host "✗ Error: $_" -ForegroundColor Red
}

Write-Host 'All events sent!' -ForegroundColor Cyan
