...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (liveMode && wsIncidents.length > 0) {
      ...
      // Update metrics
      const activeCount = wsIncidents.filter((i: any) => i.status === 'active').length;
      setMetrics((prev) => ({
        ...prev,
        active: prev.active + activeCount,
      }));
    }
